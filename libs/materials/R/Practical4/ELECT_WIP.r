...
elect<-function(model, b.covariates, statedistdata, time.scale.msm="years",
                 h, age.max, S=0, setseed=NULL, RestrAndConst=NULL, method="step"){
...
if(!method%in%c("step","Simpson","MiddleRiemann")){
    stop("\nERROR. LEs not computed. <method> should be <step>, <MiddleRiemann> or 
     <Simpson>.\n\n")
} 
...
if(method=="MiddleRiemann"){grid<-grid+h*scale/2}
...
msm.le.internal<-function(beta=beta){
  ...
  if(method=="step"){for(i in 1:length(LE)){LE[i]<- sum((h*scale)*e[,i])}}
  # Middle Riemann sum:
  if(method=="MiddleRiemann"){for(i in 1:length(LE)){LE[i]<- sum((h*scale)*e[,i])}}
  # Simpson's rule approximation of integral:
  if(method=="Simpson"){
